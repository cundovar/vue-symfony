    ...
    <div class="w-1/2 max-2xl:w-full">
      ...
      <!-- <div class=" w-full  max-2xl:w-full mt-10 mb-10">
       ...
       Exercices Grid 
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-3 gap-6 2xl:grid-cols-4">
          <router-link
            v-for="(groups, category) in filteredMenusByCategory" 
            :key="category"
...
// =============================================================================
// STORE NIVEAU - Filtrage global par niveau de cours
// =============================================================================
// Ce store est PARTAGÉ avec AppHeader.vue et App.vue
// Quand l'utilisateur change le niveau dans le header, cette page se met à jour
// automatiquement grâce à la réactivité de Vue + Pinia
import { useNiveauStore } from "@/stores/niveauCoursStore";

// Catégories à exclure de l'affichage (non pertinentes pour la homepage)
const filteredCategories = ["wordpress", "vue", "react"];

// Données depuis l'API
const { fetchMenus, cats, menus, fetchExoContents, exoContents } = useData();

// Connexion au store niveau
// selectedNiveau: le niveau sélectionné dans le header (ex: "Junior" ou null)
// C'est une ref réactive - quand elle change, tous les computed se recalculent
...
// =============================================================================
// COMPUTED: menusByCategory - Toutes les catégories (sans filtre niveau)
// =============================================================================
...
// =============================================================================
// COMPUTED: filteredMenusByCategory - Catégories FILTRÉES par niveau
// =============================================================================
// C'est ici que la magie opère!
// - Si selectedNiveau est null (pas de filtre) → retourne toutes les catégories
// - Si selectedNiveau est "Junior" → retourne seulement les catégories Junior
//
// RÉACTIVITÉ AUTOMATIQUE:
// Quand l'utilisateur change le niveau dans AppHeader.vue:
// 1. selectedNiveau change dans le store Pinia
// 2. Cette computed se recalcule automatiquement (dépend de selectedNiveau)
// 3. Le template se met à jour avec les nouvelles catégories filtrées
const filteredMenusByCategory = computed(() => {
  // DEBUG: afficher l'état actuel du filtre
  console.log('🔍 filteredMenusByCategory - selectedNiveau:', selectedNiveau.value)
  console.log('🔍 menusByCategory count:', Object.keys(menusByCategory.value).length)

  // Pas de niveau sélectionné = afficher toutes les catégories
  if (!selectedNiveau.value) {
    console.log('✅ Pas de filtre niveau, affichage de toutes les catégories')
    return menusByCategory.value;
  }

  const result = {};

  // Filtrer les catégories par niveau (par ordre, pas par nom)
  Object.entries(menusByCategory.value).forEach(([categoryName, groups]) => {
    // Récupérer le niveauCours de la catégorie via un de ses menus
    const firstMenu = menus.value.find(m => m.category?.name === categoryName);
    const catOrdre = firstMenu?.category?.niveauCours?.ordre;
    const selectedOrdre = selectedNiveau.value?.ordre;
    const hasNoNiveau = !firstMenu?.category?.niveauCours?.name;

    console.log(`  - ${categoryName}: catOrdre=${catOrdre}, selectedOrdre=${selectedOrdre}, hasNoNiveau=${hasNoNiveau}`)
    ...
    if (catOrdre <= selectedOrdre || hasNoNiveau) {
      result[categoryName] = groups;
    }
  });

  console.log('✅ Catégories après filtre:', Object.keys(result))
  return result;
});
...
const getCategoryIconClass = (categoryName) => {
  // D'abord, chercher l'icône dans l'API via les menus
  const menuWithCategory = menus.value.find(
    m => m.category?.name?.toLowerCase() === categoryName.toLowerCase()
  )

  // Si l'icône existe dans l'API (category.logo.logo), l'utiliser
  if (menuWithCategory?.category?.logo?.logo) {
    return menuWithCategory.category.logo.logo
  }
  ...
  return iconClasses[categoryName] || 'fas fa-code';
};
